<template>
  ...
  <transition name="fade">
    <div v-if="isOpenMenu" class="fixed inset-0 bg-white z-50 flex flex-col">
      <!-- close button -->
      ...
      <ul class="flex flex-col text-left space-y-4 mt-8">
        <div class="flex justify-center w-full">
          <NavbarLogin :openLabel="openLabel" @toggle="handleToggle" />
        </div>
        <li v-for="(item, idx) in menuItems" :key="idx">
          ...
          <!-- Sub items -->
          <ul
            v-show="openSubmenus[idx]"
            class="flex flex-col pl-6 mt-2 space-y-2"
          >
